...
3. Invoke the review skill on the new PR:
   - Primary: `/rhize-devflow:review` (prod merge-gate orchestrator — routes the diff to ecc specialist + security reviewers, returns one merge verdict)
   - Fallback (if `/rhize-devflow:review` is unavailable): `/pr-review-toolkit:review-pr`, `/code-review:code-review`, or `/review`
4. Cover at minimum:
