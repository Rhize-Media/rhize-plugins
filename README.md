...
| [project-launcher](./project-launcher) | 1.8.4 | 2 | Walks a new project from a rough idea through research, requirements, a written plan, and a ready-to-build project folder — for anyone starting a new software or automation project. | [README](./project-launcher/README.md) · [GUIDE](./project-launcher/GUIDE.md) |
| [rhize-devflow](./rhize-devflow) | 2.21.1 | 9 | Rhize Media's software delivery workflow — plan a change, build it, test it, and get it independently reviewed before shipping — for developers building production Next.js, Sanity, and Vercel applications. | [README](./rhize-devflow/README.md) · [GUIDE](./rhize-devflow/GUIDE.md) |
| [rhize-context-manager](./rhize-context-manager) | 0.31.1 | 16 | Keeps Claude's memory and working context organized across long sessions so information isn't lost or repeated — for anyone running long or complex Claude sessions. | [README](./rhize-context-manager/README.md) · [GUIDE](./rhize-context-manager/GUIDE.md) |
| [rhize-ops](./rhize-ops) | 0.24.0 | 3 | Rhize Media's internal operations toolkit — hands off work to teammates with full context, tracks which skills are actually earning their keep, and helps run multiple Claude agents safely at once. | [README](./rhize-ops/README.md) · [GUIDE](./rhize-ops/GUIDE.md) |
...
| [procedural-memory](./procedural-memory) | 0.5.6 | 2 | Lets Claude find and reuse previously verified scripts and automations instead of rebuilding them from scratch each time — for developers who want proven code reused safely. | [README](./procedural-memory/README.md) · [GUIDE](./procedural-memory/GUIDE.md) |
<!-- SKILL-MAP:END -->
...
**rhize-tasks** is a macOS-local service whose runtime the setup wizard fetches from its own repository, [`Rhize-Media/rhize-tasks`](https://github.com/Rhize-Media/rhize-tasks), at a pinned release (so an upgrade or rollback is a tag, not a copy). It requires Git, macOS 14+, Node.js 22+, a compatible Swift/Xcode toolchain, Apple Reminders permission, and credentials for the connector scopes the end user approves. It stores secrets only in Keychain and keeps Calendar/Reminders writes inside dedicated approved containers. See its [technical README](./rhize-tasks/README.md) and [user guide](./rhize-tasks/GUIDE.md).
...
| rhize-ops | `Rhize-Media/rhize-skill-monitor` checkout (`RHIZE_SKILL_MONITOR_ROOT`) | data | No | `skill-dashboard` stops with the clone command; nothing else in rhize-ops needs it |
| rhize-tasks | Git plus the pinned runtime release from `Rhize-Media/rhize-tasks`; macOS 14+, Node.js 22+, Swift/Xcode toolchain | platform/runtime | Yes | The local service and signed EventKit helper cannot be installed |
