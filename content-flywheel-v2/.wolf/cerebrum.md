...
- [2026-04-09] SEO Utils integration — Option B (dual-stack): DataForSEO stays as Vercel runtime client (serverless can't reach localhost). SEO Utils MCP used for Claude Code sessions only — provides GSC data, indexing checks, autocomplete scraping, content/backlink gap, automations. Option C (VPS-hosted SEO Utils for Vercel access) planned as future milestone. SEO Utils tools: query_gsc, query_database, list_tables, get_keyword_suggestions, fetch_serp_data, get_backlink_summary, get_referring_domains, get_content_gap, get_backlink_gap, check_google_indexing_status, submit_url_for_google_indexing, submit_url_to_index_now, fetch_autocomplete_keywords, check_keyword_metrics, create_automation, get_organic_keywords, get_top_pages, get_traffic_summary, get_historical_rank, get_traffic_competitors, bulk_backlink_analysis, bulk_traffic_analysis, send_email + more.
