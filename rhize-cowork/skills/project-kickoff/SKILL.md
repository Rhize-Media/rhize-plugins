...
description: Scaffold the standard project context files at the start of a new Cowork project — CLAUDE.md (operating manual), BUSINESS.md (the business/offer/market), PERSONALITY.md (brand voice & tone), and INFO.md (links, tools, people). Use this whenever the user starts a new project, says "set up a new project," "kick off," "initialize this project," "create the project files / CLAUDE.md," onboards a new client, or drops a website/strategy doc and wants context files built. Works from a website, from strategy docs, or from nothing via a guided interview. Trigger even if the user only names one file — build the full set unless told otherwise.
---
