...
export function projectTodayView({plan, tasks = [], operations = [], profile, freshness = {}, now = new Date().toISOString(), approvedOutsideLabels = {}}) {
  ...
  const approvals = operations.filter(operation => operation.approval === 'required').map(operation => ({operationId: operation.id, kind: operation.kind, title: tasks.find(task => task.id === operation.targetId)?.title ?? operation.kind, reason: 'approval_required'}));
  ...
  return clone({schemaVersion: 1, planRevision: plan.planRevision, generatedAt: plan.generatedAt ?? now, timeline: items, currentBlock, nextBlock, capacity: {availableMinutes: plan.availableMinutes ?? 0, plannedMinutes: plan.usedMinutes ?? 0, bufferMinutes: plan.bufferMinutes ?? 0, risk: (plan.usedMinutes ?? 0) > (plan.capacityMinutes ?? 0) ? 'over' : (plan.capacityMinutes ?? 0) - (plan.usedMinutes ?? 0) < (plan.bufferMinutes ?? 0) ? 'tight' : 'normal'}, carryovers, approvals, opportunities, warnings, connectors, paused, degraded: systems.some(system => connectors[system].status !== 'healthy')});
}
