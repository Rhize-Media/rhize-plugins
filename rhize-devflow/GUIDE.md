...
**Heavier guard hooks (duplicate-check, prewrite mutation check, RCA enforcer, regression guard) don't seem to be running:** They're opt-in by design — they ship under each skill's `hooks/` directory but aren't wired into any project's `.claude/settings.json` automatically. Wire in the ones you want per-project rather than expecting them to fire out of the box.
