...
- **Drift check:** `rhize-devflow:simplify` is the canonical operating contract. Claude built-in version movement is informational provenance only; it does not require replaying the native command or block this Rhize-owned workflow. Reassess the explicit Rhize contract only when a user requests an outcome it cannot safely satisfy.
