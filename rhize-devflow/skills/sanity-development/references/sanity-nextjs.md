...
```typescript
import { defineLive } from 'next-sanity/live'  // ✅ v11+ import path
...
export async function sanityFetch<const QueryString extends string>({
  ...
  return client.fetch(query, params, {
    cache: 'force-cache',  // ✅ Required in Next.js 15 (no longer default)
    next: {
...
## 7. Perspectives & Preview

When fetching draft content, use the `perspective` option:

| Perspective | Description | Use Case |
|-------------|-------------|----------|
| `published` | Only published documents (default) | Production |
| `drafts` | Prioritizes drafts over published | Preview mode, Visual Editing |
| `previewDrafts` | Same as `drafts` (legacy name) | Still works, prefer `drafts` |
| `raw` | Returns both drafts and published (deprecated) | Avoid |

```typescript
// Preview client for draft content
const previewClient = client.withConfig({
  perspective: 'drafts',  // ✅ Modern syntax
...
### Production Pattern with Sentry
