...
This skill turns work from Jim's current session into a clearly structured handoff package for [REDACTED_NAME], who handles advertising, marketing, sales, and increasingly technical tasks across the Rhize Media stack.

the recipient is sharp with marketing, ads, and sales — but he's still building his technical muscles. He's learning Claude Code/Cowork, Vercel, Sanity CMS, Next.js basics, and the DataForSEO APIs. The instructions you generate for him should be thorough and encouraging, never assuming deep technical knowledge. Think of it as writing instructions for a capable colleague who needs the "why" and "how" spelled out clearly, not just the "what."

## When This Skill Triggers

Any time Jim wants to hand off work to the recipient. Common phrasings:
- "delegate this to the recipient"
- "the recipient should handle this"
- "create tasks for the recipient"
- "hand this off"
- "assign to the recipient"
- Just "delegate" (the recipient is the default)
...
- Capture any file paths, URLs, project names, or technical details that the recipient will need
...
5. Include a **Meeting Context** section in the task package with:
   ...
   - Any specific quotes or requirements that the recipient needs to be aware of
...
## Why This Matters
[Brief context on why this task is important — business impact, client need, deadline driver. the recipient does better work when he understands the bigger picture.]
...
Beyond what was used in Jim's session, think about what else would help the recipient:
...
- Think about the recipient's skill level — recommend tools that will make the task easier, not harder
...
Use the Jira MCP to create issues for each task. Use the Atlassian cloud ID `[REDACTED_ATLASSIAN_CLOUD_ID]`.
...
- **Assignee:** [REDACTED_NAME] — account ID `[REDACTED_JIRA_ACCOUNT_ID]`
...
The recipient cannot access Jim's local Obsidian vault. If relevant vault documents were found during Step 1 context gathering, share them with the recipient as Slack Canvases.
...
4. Share the Canvas in the `#[REDACTED_CHANNEL]` channel (Channel ID: `[REDACTED_SLACK_CHANNEL_ID]`) by including the Canvas link in the main Slack message (Step 8). **Do NOT send via DM** — keep everything co-located in #[REDACTED_CHANNEL] with the rest of the recipient's task notifications.
...
- Prioritize sharing documents that the recipient will actively use (checklists, SOPs, reference docs) over background context docs
...
- Always include the Canvas link in both the Jira ticket comment AND the Slack message to the recipient
...
Post a structured delegation to the `#[REDACTED_CHANNEL]` Slack channel (Channel ID: `[REDACTED_SLACK_CHANNEL_ID]`) using a **main message + thread replies** pattern. This keeps the channel scannable while giving the recipient full context in-thread.

**Always tag the recipient using `<@[REDACTED_SLACK_MEMBER_ID]>`** so he gets a notification.
...
```
:clipboard: *New Tasks for <@[REDACTED_SLACK_MEMBER_ID]>*
...
## the recipient's Technical Context

When writing instructions for the recipient, keep these in mind:

**What the recipient knows well:**
- Advertising platforms and strategy
- Marketing campaigns and content
- Sales processes and outreach
- Business strategy and client relations

**What the recipient is learning:**
...
**Writing tone for the recipient's instructions:**
- Direct and encouraging — the recipient is capable, he just needs the technical bridges
...
- **Cloud ID:** `[REDACTED_ATLASSIAN_CLOUD_ID]`
- **the recipient's Account ID:** `[REDACTED_JIRA_ACCOUNT_ID]`
- **the recipient's Email:** `[REDACTED_WORK_EMAIL]`
- **Atlassian URL:** `https://[REDACTED_ATLASSIAN_TENANT].atlassian.net`

## Slack Configuration Reference

- **Target Channel:** `#tom-tasks` (Channel ID: `[REDACTED_SLACK_CHANNEL_ID]`)
- **the recipient's Slack User ID:** `[REDACTED_SLACK_MEMBER_ID]` (use `<@[REDACTED_SLACK_MEMBER_ID]>` to tag him)
