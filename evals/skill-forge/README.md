...
local scan latency across explicit repetitions. Both commands use a temporary home and an explicit
environment allowlist so ambient credentials and user configuration do not reach the executable.
Results stay outside Git; imperfect precision or recall is a measured finding, not silently
converted into a failed harness run.
...
repetitions. Validation requires all three complete pre/post pairs with the same two digests, then
gates correctness, routing precision/recall, latency, rework, and failures. The protocol is isolated,
offline, and never adopts a proposal. No receipts or claimed results are included because the
agent-mediated evaluation has not been run.
