...
- `run_local_evals.py` is the immediate deterministic, offline gate. It makes no model,
  network, DataForSEO, or paid calls.
...
The local routing score is deliberately limited: it checks curated substrings from each
live `SKILL.md`, keyword drift, one positive and two documented near-miss/collision
negatives per skill, complete live-quality coverage, and static operational contracts.
It does **not** claim to measure a model's real Skill invocation decision.
