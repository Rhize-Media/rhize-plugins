#!/usr/bin/env python3
"""setup_orchestrator.py — deterministic backend for /rhize-core:setup (R2, the hybrid
setup wizard; `/rhize-ops:rhize-setup` forwards here for one release). `/rhize-core:setup`
(the command .md) keeps only interaction — questions,
...
RHIZE_SIGNATURE_PLUGIN = "rhize-core"
# Pre-repo-shape-R-B installs only ever shipped rhize-ops, and the rhize-ops fallback
# copy of this script must keep recognising a marketplace that hasn't picked up
# rhize-core yet -- so a clone with EITHER plugin present is a valid rhize-plugins root.
RHIZE_SIGNATURE_FALLBACK_PLUGIN = "rhize-ops"
...
def is_rhize_marketplace(root: Path) -> bool:
    return (
        (root / RHIZE_SIGNATURE_PLUGIN / ".claude-plugin" / "plugin.json").is_file()
        or (root / RHIZE_SIGNATURE_FALLBACK_PLUGIN / ".claude-plugin" / "plugin.json").is_file()
    )
