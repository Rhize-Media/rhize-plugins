...
the managed section of rhize-core/docs/setup-artifacts.md (hybrid-setup-wizard.md R2 §5;
repo-shape R-B moved this from rhize-ops to rhize-core, see doc_relative_path()).
...
def resolve_repo_root(home: Path | None = None) -> Path:
    ...
    cache (`~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/scripts/`) it is not, so
    fall back to the marketplace clone under `~/.claude/plugins/marketplaces/` that contains
    either the `rhize-core` plugin or (pre-repo-shape-R-B / this fallback copy without
    rhize-core installed) the `rhize-ops` plugin. `--repo-root` overrides both.
    """
    ...
    if base.is_dir():
        for candidate in sorted(base.iterdir()):
            if (candidate / ".claude-plugin" / "marketplace.json").is_file() and (
                (candidate / "rhize-core").is_dir() or (candidate / "rhize-ops").is_dir()
            ):
                return candidate
    return checkout


def doc_relative_path(repo_root: Path) -> Path:
    """Resolve the rendered docs/setup-artifacts.md path (same preference rule as
    evaluation_setup.catalog_relative_path): rhize-core's doc when a `rhize-core` plugin
    directory exists under `repo_root`, else this copy's own plugin directory (derived
    from where this file lives on disk), so a rhize-ops-only install stays self-contained."""
    if (repo_root / "rhize-core").is_dir():
        return Path("rhize-core") / "docs" / "setup-artifacts.md"
    own_plugin = Path(__file__).resolve().parents[1].name
    return Path(own_plugin) / "docs" / "setup-artifacts.md"
...
def render(repo_root: Path) -> bool:
    return apply_markers(repo_root / doc_relative_path(repo_root), render_table(collect_rows(repo_root)))
...
def _materialize_inputs(repo_root: Path, root: Path) -> None:
    ...
    doc_relative = doc_relative_path(repo_root)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if changed:
            print(
                ...
                f"python3 {doc_relative.parts[0]}/scripts/setup_artifacts.py --markdown and commit the result",
                file=sys.stderr,
            )
            return 1
