...
/graphify <path> --neo4j                              # governed graph-memory validate + preview (no Cypher)
/graphify <path> --neo4j-push bolt://localhost:7687   # refused for governed data; live canary is RT-159
...
- Never use Graphify's direct Neo4j export or push for Rhize governed data; hand `graph.json` to `graph-memory`.
