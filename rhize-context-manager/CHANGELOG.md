...
- `/skill-refine run`'s `evolve` invocation now passes `--backend claude`
  ...
  Verified end-to-end with a real dry-run (6 sessions, 5 mined tasks, baseline
  0.29 → candidate 0.63, 4 genuine proposed edits). Requires `skillopt-sleep`
...
- `/learn-harvest`'s noise filter reference set (both the command and the
  ...
  MEMORY.md sections back as apparent new findings); adding it moved 21 of 41
