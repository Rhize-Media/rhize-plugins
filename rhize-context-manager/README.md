...
| `/memory-context` | Assemble and verify a private scoped preview over explicit supported adapters, or use opt-in `catalog` → `expand` for source-bound topic awareness and selected details. Combined budgets include catalog metadata; revision, digest, authority, scope, TTL and purge remain enforced. [Input/usage contract](skills/memory-context/references/awareness.md). No automatic injection or write-back. |
