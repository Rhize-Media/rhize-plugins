...
- **Evidence:** Claude Code auto-wires the selector/finalizer through `hooks/hooks.json`; Codex does not consume that Claude hook manifest and invokes the same host-neutral runners explicitly through the canonical skill. Duplicate manually installed Claude hooks remain a migration concern.
...
- Auto-wire the fail-silent selector and finalizer for Claude Code, and expose the identical host-neutral runners for explicit Codex skill invocation. Duplicate manual Claude hook entries remain safe/idempotent but must be removed by the coordinator during migration.
...
| rhize-plugins | `hooks/hooks.json`, selector/finalizer wrappers | Claude hook registration plus shared host-neutral runners used explicitly by Codex | hook manifests and wrapper tests |
...
- Claude auto-wired selector/finalizer hook entries parse, resolve to existing wrappers, and remain duplicate-safe; Codex discovery points to the same explicit runners without claiming hook registration.
...
- **Evidence:** Codex plugin discovery consumes canonical skills, not Claude's `hooks/hooks.json`; repository tests can prove skill/runner discovery and Claude hook shape, while a fresh installed Claude session is still required to prove host hook registration.
...
- `IN_SYNC_WITH_EXCEPTIONS`: implementation, focused/full tests, documentation, and
  the final `rg`-based diff reconciliation agree. A fresh installed Claude session
  is still required to prove hook registration; Codex uses explicit skill invocation. This repository has
